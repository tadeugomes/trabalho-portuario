...
rais %>% 
  ...
  geom_point(
    data = . %>%  group_by(grau_instrucao_apos_2005) %>% filter(remuneracao_media == max(remuneracao_media)),
  ...
  #           big.mark = ".")) +
  scale_y_continuous(
    labels = label_number(scale = 1/1000, big.mark = ".", decimal.mark = ",",  accuracy = 0.1)) +
...
seguir. Nota-se o predomínio de pardos e brancos, tanto entre os
...
rais %>%
  ...
  scale_y_continuous(
    labels = label_number(
      scale = 1/1000, big.mark = ".", decimal.mark = ",", accuracy = 0.1)) +
...
grupo_ma %>%
  ...
  scale_y_continuous(
    labels = label_number(scale = 1/1000, big.mark = ".", decimal.mark = ",",  accuracy = 0.1)) +
    labs(title = "Remuneração média no Maranhão por Cor ou Raça",
    x = "Ano", y = "Remuneração média (R$)", color = "")+
  facet_wrap(~sexo, ncol = 2) +
  theme_classic() +
  theme(plot.title = element_text(size = 16L, face = "bold"),
...
Verifica-se que a remuneração média dos trabalhadores portuários e aguaviários varia conforme o estado da federação. O Rio de Janeiro é o estado com a maior remuneração no período analisado, com o registro de R$ 8.535 em 2020. O Espírito Santo aparece em segundo lugar, com R$ 6.012. 
