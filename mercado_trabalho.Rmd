...
virgula <- function(x) format(x, digits = 1, big.mark = ".", decimal.mark = ",")
```
...
grafico(
 ...
 geom_text_repel(
    data = . %>% filter(ano == 2020),
    aes(x = ano , y = n,
        label = str_wrap(grau_instrucao_apos_2005, 28),
...
ggthemes::scale_color_gdocs() +
  bbc_style() +
   scale_x_discrete(expand = expansion(mult = c(0,.8))) +
   theme(
     legend.position = "none",
     panel.grid.major.y = element_blank(),
     axis.text.y = element_blank(),
...
rais %>%
  ...
  #   labels = label_number(scale = 1/1000, big.mark = ".", decimal.mark = ",")) +
    labs(x = "Ano", y = "Número de Vínculos", color = "Raça ou cor",
         caption = "Fonte: Observatório Portuário - Dados da RAIS") +
  facet_wrap(~sexo,ncol = 2, scales = "fixed")+
  theme_classic() +
