...
Atividades Auxiliares dos Transportes** e **Transporte Aguaviário**.
...
vínculos em 2020 na categoria (18.237). A categoria abarca atividades
como:
...
-   A **Navegação de Apoio Marítimo**, por sua vez, duplicou o número de
    vínculos e chegou a 15.372 profissionais na área.
...
<br>
Os dados são um reflexo das políticas de regulação para o setor, evidenciando como as estratégias governamentais e empresariais se refletem no estoque de empregos no setor. 
>>>>>>> d6ee7ae0e7605e7000ef78abfd128886e9f4badd
...
<<<<<<< HEAD
  ...
  geom_text_repel(
        data = . %>% group_by(n_grupo) %>% filter(n == max(n)),
        aes(
          x = ano, y = n,
            label = scales::number(
            n,
  ...
  ggthemes::scale_color_gdocs() +  
  scale_colour_discrete(name = str_wrap("Divisões da CNAE", 10), labels = label_wrap_gen(width = 30)) 
=======
...
resultado, saltando de 14 para 920 vínculos, o que indica uma
...
apesar do número de profissionais com pós-graduação ainda ser baixo. Em
2020, a quantidade de ocupações que exigiam o **ensino médio completo**
era de 58,57% (desses 91,25% eram do sexo masculino e apenas 8,75% do sexo
feminino), por outro lado, 21,4% das ocupações exigiam o **Superior
completo** (desses 71,19% eram do sexo masculino e 28,81% do sexo feminino)
...
grafico(
  ...
  nivel2 = grau_instrucao_apos_2005, 
<<<<<<< HEAD
  titulo = "Quantidade de Vínculos por grau de escolaridade e sexo - Brasil (em mil)"
  ) +
     geom_text_repel(
      data = . %>% filter(ano %in% c("2010", "2020")),
      aes(
  ...
  geom_text_repel(
        data = . %>% group_by(sexo, grau_instrucao_apos_2005) %>% filter(n == max(n)),
        aes(
          x = ano, y = n,
            label = scales::number(
            n,
...
grafico(
  ...
  nivel2 = grau_instrucao_apos_2005, 
<<<<<<< HEAD
  titulo = "Quantidade de Vínculos por grau de escolaridade e sexo - Maranhão" ) +
  scale_y_continuous(labels = label_number(scale = 1/1000)) +
  ggthemes::scale_color_gdocs() +
     geom_text_repel(
      data = . %>% filter(ano %in% c("2010", "2020")),
...
<<<<<<< HEAD
seguir. Nota-se o predomínio de pardos e brancos, tanto entre os homens
...
<<<<<<< HEAD
```{r g_operacao_remuneracao _media_raca_sexo, results='hide', echo=FALSE, message=FALSE, warning=FALSE}
=======
...
rais %>%
  ...
  #   labels = label_number(scale = 1/1000, big.mark = ".", decimal.mark = ",")) +
    labs(title = "Remuneração Média no Brasil por Cor ou Raça",
         x = "Ano", y = "Remuneração mediana (R$)", color = "")+
  facet_wrap(~sexo, ncol = 2) +
  theme_classic() +
  theme(plot.title = element_text(size = 16L, face = "bold"),
        plot.caption = element_text(size = 12L,face = "bold", hjust = 0), 
...
A renda dos trabalhadores por cor ou raça no Maranhão, por sua vez, evidencia que os autodeclarados amarelos se destacam entre os homens, com remuneração mediana acima das demais (amarelos, de acordo com o IBGE, são aqueles que se declaram de origem asiática: japoneses, coreanos e chineses). 
>>>>>>> d6ee7ae0e7605e7000ef78abfd128886e9f4badd

```{r g_operacao_remuneracao _mediana_raca_sexo.ma, results='hide', echo=FALSE, message=FALSE, warning=FALSE}

grupo_ma %>%
  filter(valor_remuneracao_media != 0.00 | !is.na(valor_remuneracao_media) | raca_cor != 'Não identificado') %>%
  group_by(ano, raca_cor, sexo, .groups = 'drop') %>% 
  summarise(n = n(),
            media = median(valor_remuneracao_media)) %>% 
  mutate(ano = factor(ano, lvls_ano)) %>%
  ...
  # scale_y_continuous(
  #   labels = label_number(scale = 1/1000, big.mark = ".", decimal.mark = ",")) +
    labs(title = "Remuneração mediana no Maranhão por Cor ou Raça",
    x = "Ano", y = "Remuneração mediana (R$)", color = "")+
  facet_wrap(~sexo, ncol = 2) +
  theme_classic() +
  theme(plot.title = element_text(size = 16L, face = "bold"),
...
=======
Verifica-se que a remuneração mediana dos trabalhadores portuários e aguaviários varia conforme o estado da federação. O Rio de Janeiro é o estado com a maior remuneração no período analisado, com o registro de R$ 8.535 em 2020. O Espírito Santo aparece em segundo lugar, com R$ 6.012. 

O estado do Maranhão teve uma evolução signifitiva no valor da remuneração mediana: saltou de R$ 2.090 em 2010 para R$ 4.765 em 2020. 
>>>>>>> d6ee7ae0e7605e7000ef78abfd128886e9f4badd



```{r g_operacao_estado, echo=FALSE, message=FALSE, warning=FALSE, results='hide', include=FALSE}


# Remuneracao com valor nominal. Não plotar. 
rais %>% 
...
lvls <- 
  rais %>% 
  group_by(ano, nome_uf) %>% 
  summarise(media = median(valor_remuneracao_media)) %>% # mediana da remuneracao
  filter(ano == 2020) %>% 
  slice_max(media, n = 8) %>% 
  select(nome_uf) %>% 
  pull()
...
rais %>% 
    filter(valor_remuneracao_media != 0.00 | !is.na(valor_remuneracao_media)) %>%
  group_by(ano, nome_uf) %>% 
  summarise(media = median(valor_remuneracao_media)) %>% # mediana
  mutate(ano = factor(ano, lvls_ano)) %>%
 ...
 geom_text_repel(
  ...
  theme_classic() +
   theme(
    ...
    strip.background = element_blank()) +
  labs(x = "Ano", y = "Remuneração mediana (em mil)", color = "",
...
rais %>% 
  ...
  theme(plot.title = element_text(size = 16L, face = "bold"),
        ...
        legend.justification = c("left", "top"),
        legend.title = element_text(size = 16L, face = "bold")) +
  labs(x = "Ano", y = "Remuneração média (em mil)", color = "",
         caption = "Fonte: Observatório Portuário - Dados da RAIS") +
    scale_colour_discrete(name = str_wrap("Grupos da CNAE", 10), labels = label_wrap_gen(width = 18)) +
  scale_y_continuous(labels = label_number(scale = 1/1000)) +
    scale_colour_discrete(name = str_wrap("Grupos da CNAE", 10), labels = label_wrap_gen(width = 18)) +
...
vínculos. Em 2020, último ano da série histórica, foram
`r uf %>% group_by(ano) %>% summarise(total = sum(n)) %>% filter(ano == "2020") %>% select(total) %>% pull() %>% virgula()`
São Paulo e Rio de Janeiro permanceram na primeira e segunda coloção
durante toda a série. Em 2020, por exempo, **Rio de Janeiro** possuía
