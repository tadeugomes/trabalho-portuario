...
>>>>>>> 2c04a5e58dd562baeacfae0ad2f355c4b5e45a27
...
<<<<<<< HEAD
  ...
  labs(x = "Ano", y = "Número de vínculos (em mil)",
       title = "Número de Vínculos do Setor Portuário e Aguaviário no País (2010 - 2020)", subtitle = "Seção H da CNAE",
...
<<<<<<< HEAD
```{r g_subclasse, echo=FALSE, message=FALSE, warning=FALSE, fig.height=14, fig.width=14}
subclasse <- 
  manipulacao(
    dataset = rais, 
    nivel1 = n_classe, 
...
rais %>%
  ...
  geom_point(
    data = . %>%  
      ...
      color = "red", size = 2) +
  geom_text_repel(
  data = . %>% group_by(grau_instrucao_apos_2005) %>% filter(n == max()),
    aes(label = scales::number(n, scale = 1/1000,  decimal.mark = ",", big.mark = ".", accuracy = .01, suffix = " mil")), 
  min.segment.length = unit(0.2, "lines"),size = 3, color = "black") +
  geom_text_repel(
  data = . %>% 
    group_by(grau_instrucao_apos_2005) %>% 
    filter(ano %in% c(2010, 2020)),
  aes(label = scales::number(n, scale = 1/1000,  decimal.mark = ",", big.mark = ".", accuracy = .01, suffix = " mil")), 
  min.segment.length = unit(0.2, "lines"), size = 3, color = "black") +
