...
atividades em plataformas marítimas e de repação naval que movimentam inúmeras cadeias produtivas e setores econômicos.

Espera-se que as ss informações apresentadas (a metodologia está disponível na última seção) possam auxiliar o debate no setor e subsidiar a elaboração de políticas de gestão públicas e privadas. 

Este é o primeiro de uma série de quatro relatórios sobre o mercado de trabalho portuário e aquaviário brasileiro e maranhense. 
...
**Armazenamento e Atividades Auxiliares dos Transportes** e 
...
Aquaviário\*\*, cujas atividades são as relacionadas aos transportes de
...
vínculos em 2020 na categoria (18.237). A categoria abarca atividades como:
...
finalizou 2020 com 11.488 vínculos. São atividades como *o
...
seções da Classificação Nacional de Atividades Econômicas (CNAE) do Instituto Brasileiro de Geografia e Estatística (IBGE), o que permite identificar quais áreas impactaram
diretamente o estoque de empregos na área portuária e aquaviária.
...
-   Ao considerar a **Gestão de Portos e Terminais**, o destaque foi
    ...
    tiveram um declínio de 41,25%, resultado da destruição de 2.170 vínculos;
...
apresentavam maior escolaridade no contexto nacional: 44,5% tinham curso superior, contra 17,7%
...
```{r g_operacao_remuneracao _mediana_raca_sexo, results='hide', echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
rais %>%
  filter(valor_remuneracao_media != 0.00 | !is.na(valor_remuneracao_media)) %>%
  group_by(ano, raca_cor, sexo, .groups = 'drop') %>% 
  summarise(n = n(),
  ...
  # scale_y_continuous(
  #   labels = label_number(scale = 1/1000, big.mark = ".", decimal.mark = ",")) +
    labs(title = "Remuneração Mediana no Brasil por Cor ou Raça",
...
grupo_ma %>%
  ...
  # scale_y_continuous(
  #   labels = label_number(scale = 1/1000, big.mark = ".", decimal.mark = ",")) +
    labs(x = "Ano", y = "Remuneração média no MA (R$)", color = "Raça ou cor",
         caption = "Fonte: Observatório Portuário - Dados da RAIS") +
  facet_wrap(~sexo, ncol = 2) +
  theme_classic() +
  theme(plot.title = element_text(size = 16L, face = "bold"),
...
Verifica-se que a remuneração mediana dos trabalhadores portuários e aquaviários varia conforme o estado da federação. O Rio de Janeiro é o estado com a maior remuneração no período analisado, com o registro de R$ 5.668,50 em 2020. O Espírito Santo aparece em segundo lugar, com R$ 4.584,20.  

O estado do Maranhão teve uma evolução signifitiva no valor da remuneração mediana: saltou de R$ 1.257,90 em 2010 para R$ 2.803.50 em 2020.
...
Ao analisar a remuneração média por município, o destaque positivo fica com o município do Rio de Janeiro, juntamente com Santos, que pagam as maiores remunerações, acima de R$ 8.500,00. 
...
Importante destacar que os dados dos Órgãos Gestores de Mão de Obra (OGMOs) não foram analisados e incorporados a esse relatório, pois o serão nos próximos boletins. 
